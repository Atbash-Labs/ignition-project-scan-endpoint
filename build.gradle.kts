...
ignitionModule {
     ...
     */
    name.set("Project Scan Endpoint")

    /*
     * Name of the '.modl' file to be created, without file extension.
     */
    fileName.set("Project-Scan-Endpoint.modl")
    /*
     * Unique identifier for the module.  Reverse domain convention is recommended (e.g.: com.mycompany.charting-module)
     */
    id.set("project-scan-endpoint")
    license.set("LICENSE.txt")
    moduleVersion.set(version.toString())

    moduleDescription.set("A module provideing a REST endpoint for scanning the project for various resources.")
    /*
     ...
     */
    hooks.set(mapOf(
        "com.bwdesigngroup.ignition.project_scan.gateway.ProjectScanEndpointGatewayHook" to "G",
        "com.bwdesigngroup.ignition.project_scan.designer.ProjectScanEndpointDesignerHook" to "D"
    ))
...
tasks.withType<io.ia.sdk.gradle.modl.task.Deploy>().configureEach {
    hostGateway.set(project.findProperty("hostGateway") as String?)
}
